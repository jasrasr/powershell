...
# rev 2.4 - added winner animations (flashing banner for PS7+, expanding text for PS5)
# rev 2.5 - added legal play logic: small square played determines required big board for next move
...
function Initialize-Board {
    ...
    $global:RequiredBig = $null  # Tracks which big board must be played next
...
$global:WinCount = @{ "X" = 0; "O" = 0; "Draw" = 0 }
...
function Get-RandomMove {
    # If there's a required big board, try to play there first
    if ($null -ne $RequiredBig -and $BigWinners[$RequiredBig] -eq " ") {
        $smallList = @(1..9 | Where-Object { $Board[$RequiredBig][$_] -eq " " })
        if ($smallList.Count -gt 0) {
            $smallMove = Get-Random -InputObject $smallList
            return @{ Big = $RequiredBig; Small = $smallMove }
        }
    }
    
    # If required big board is full or won, play anywhere
...
while ($true) {
    ...
    # Display required board if applicable
    if ($null -ne $RequiredBig -and $BigWinners[$RequiredBig] -eq " ") {
        $hasAvailableMoves = @(1..9 | Where-Object { $Board[$RequiredBig][$_] -eq " " }).Count -gt 0
        if ($hasAvailableMoves) {
            Write-Host "`n$playerName's Turn ($currentPlayerSymbol) - MUST play in BIG square $RequiredBig (q=Quit r=Reset u=Undo)" -ForegroundColor Yellow
        } else {
            Write-Host "`n$playerName's Turn ($currentPlayerSymbol) - Free choice (required board $RequiredBig is full) (q=Quit r=Reset u=Undo)" -ForegroundColor Yellow
            $RequiredBig = $null
        }
    } else {
        Write-Host "`n$playerName's Turn ($currentPlayerSymbol) - Free choice (q=Quit r=Reset u=Undo)" -ForegroundColor Yellow
        $RequiredBig = $null
    }
    ...
    if (-not $AutoPlay -and -not $TestMode) {
        do {
            if ($null -ne $RequiredBig -and $BigWinners[$RequiredBig] -eq " ") {
                ...
                if ($hasAvailableMoves) {
                    $bigMove = $RequiredBig
                    Write-Host "Playing in required BIG square $RequiredBig" -ForegroundColor Cyan
                    break
                } else {
                    $RequiredBig = $null
                }
            ...
            if ($bigMove -eq "u" -and $MoveHistory.Count -gt 0) {
                ...
                # Update RequiredBig based on previous move
                if ($MoveHistory.Count -gt 0) {
                    $RequiredBig = $MoveHistory[$MoveHistory.Count - 1].Small
                } else {
                    $RequiredBig = $null
                }
                Show-BigBoardWinners
    ...
    # Set the required big board for next player based on small square played
    $RequiredBig = $smallMove
    ...
    if ($BigWinners[$bigMove] -eq " " -and (Check-SmallBoardWin $bigMove $currentPlayerSymbol)) {
        ...
        if (Check-FullGameWin $currentPlayerSymbol) {
            ...
            if ($PSVersionTable.PSVersion.Major -ge 7) {
                ...
                Animate-ExpandingWinner -text "WINNER!" -delay 150
                Write-Host "`n*** $($players[$currentPlayerSymbol]) wins the GAME! ***" -ForegroundColor Magenta
            }
