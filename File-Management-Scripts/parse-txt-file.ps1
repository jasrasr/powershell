...
#   (4) remove rows under a provided child-folder list (proper "\*" handling; includes your expanded list),
...
param(
    ...
    # Choose which filters to apply. If none specified (and no RemoveListFile), all enabled by default.
...
<<<<<<< HEAD
    ...
    [switch]$RemoveListIsRegex,        # Rule 9 : treat list lines as regex
    [switch]$RemoveListCaseSensitive   # Rule 9 : case-sensitive matching
)
...
<<<<<<< HEAD
  ...
  '1.13 Fee Estimate\*',
  '1.13 TPT Documents\*',
  '1.14 Project Sign Off\*',
  '1.14 Proposal Sign Off Form\*',
  ...
  '10.5 Wrk Dwgs\*',
  '10.6 Advance Steel\*',
  '10.6 Construction Package\*',
  '10.6 Discipline\*',
  '10.6 Laser Scan Data\*',
  '10.6 Scanning\*',
  '10.7 Drawings to Return for Mechanical Completion\*',
  '10.7 Exports\*',
  ...
  '3.8 iCOR\*',
  '3.8 Issued Drawings\*',
  '3.8 Proposal Sign Off Form\*',
  '3.8 Proposal Sign Off\*',
  '3.9 Client-Project Team\*',
  '3.9 ICOR\*',
  '3.9 Organization Chart\*',
  '3.9 Project Controls Cost Reports\*',
  ...
  '4.15 I-O List\*',
  '4.16 Vendor Files\*',
  '4.16 Vendor Motor Files\*',
  '4.17 Photos\*',
  '4.18 Discipline\*',
  '4.19 Change Management\*',
  '4.19 Conduit _ Cable Schedule\*',
  '4.19 Construction Administration\*',
  '4.19 ICORs\*',
  '4.19 Reservation Request\*',
  '4.19 Reservation Requests\*',
  '4.19 Service Requests\*',
  '4.19 Word template\*',
  '4.19 Word templates\*',
...
$childfolderscount = $childFolders.Count
$childfolderscount
$childFolders = $childFolders | Sort-Object -Unique
#sort a -z
$childFolders = $childFolders | Sort-Object
$childfolderscount = $childFolders.Count
$childfolderscount
$childFolders
#>
...
# -------------------------------------------
$rxGST_TripletChildren = '^\\\\middough\.local\\corp\\data\\archive\\[^\\]+\\_(Gen|Stds|Transfer|Stat)\\.+'
...
# -------------------------------------------
# Rule 9 : Optional removal list pre-compile
# -------------------------------------------
$rxRemoveList = $null
$removeListFallback = $null
if ($RemoveListFile) {
    if (Test-Path -LiteralPath $RemoveListFile) {
        $rawList = Get-Content -LiteralPath $RemoveListFile | Where-Object { $_ -and $_.Trim().Length -gt 0 }
        if ($rawList.Count -gt 0) {
            $parts = if ($RemoveListIsRegex) { $rawList } else { $rawList | ForEach-Object { [regex]::Escape($_) } }
            $rxOptions = if ($RemoveListCaseSensitive) { [System.Text.RegularExpressions.RegexOptions]::None } else { [System.Text.RegularExpressions.RegexOptions]::IgnoreCase }
            $pattern = '(?:' + ([string]::Join('|', $parts)) + ')'
            try {
                $rxRemoveList = [regex]::new($pattern, $rxOptions)
                Write-Host "Loaded remove list patterns : $($rawList.Count)" -ForegroundColor DarkCyan
            }
            catch {
                Write-Host "Failed to compile remove list regex. Falling back to literal contains checks." -ForegroundColor Yellow
                $rxRemoveList = $null
                $removeListFallback = $rawList
            }
        }
        else {
            Write-Host "Remove list is empty : $RemoveListFile" -ForegroundColor Yellow
        }
    }
    else {
        Write-Host "Remove list file not found : $RemoveListFile" -ForegroundColor Yellow
    }
}
...
<<<<<<< HEAD
    ...
    # Rule 9 : remove lines that match any pattern from -RemoveListFile
    if (-not $remove -and $RemoveListFile) {
        if ($rxRemoveList) {
            if ($rxRemoveList.IsMatch($line)) { $remove = $true }
        }
        elseif ($removeListFallback) {
            foreach ($item in $removeListFallback) {
                if ($RemoveListCaseSensitive) {
                    if ($line.Contains($item)) { $remove = $true; break }
                } else {
                    if ($line.IndexOf($item, [System.StringComparison]::OrdinalIgnoreCase) -ge 0) { $remove = $true; break }
                }
            }
        }
    }
...
>>>>>>> 958b4992f536344ed19b8eb80fdc154faabcce07
    ...
    # Rule 2 : remove if line ENDS with one of the extensions
    ...
    # Rule 3 : remove specific Newforma child folders (keep Newforma root)
...
.\parse-txt-file.ps1 'K:\101225 txt\archive-depth3.txt' -RemoveListFile 'K:\filters\to-remove.txt'
.\parse-txt-file.ps1 'K:\101225 txt\archive-depth3.txt' -RemoveListFile 'K:\filters\patterns.txt' -RemoveListIsRegex
.\parse-txt-file.ps1 'K:\101225 txt\archive-depth3.txt' -RemoveListFile 'K:\filters\to-remove.txt' -RemoveListCaseSensitive
...
# Extensions : .msg .xlsx .docx .pdf .xls .doc .pptx .csv .zip .log .txt .dcf .dcfx .xml .xml.lock .dwg .mp4 .lnk .mpg .nwc
# Kept:
\\server\share\folder\readme.md
# Removed:
\\server\share\folder\mail.msg
\\server\share\folder\model.nwc

# Rule 3 : Newforma children (keep Newforma root)
# Kept:
\\middough.local\corp\data\archive\Basf\ENG1840\Newforma
# Removed:
\\middough.local\corp\data\archive\Countrymark.CHI\Newforma\Record Copies\Transmittals
...
# Rule 9 : Remove lines matching -RemoveListFile entries
# List file examples (literal mode):
#   \\server\share\path,with,commas
#   \\middough.local\corp\data\archive\Basf\ENG1840\Photos\01-18-10 Sebek\image.jpg
#   \.mpg$                # treated as literal when NOT using -RemoveListIsRegex
# Regex mode (-RemoveListIsRegex) examples:
