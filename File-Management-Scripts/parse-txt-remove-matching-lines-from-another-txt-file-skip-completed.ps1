...
foreach ($file in $BatchFiles) {
    ...
    if (Test-Path $outFile) {
        Write-Host "`nSkipping (already cleaned) : $($file.Name)" -ForegroundColor Yellow
        continue
    }
...
if ($summary.Count -gt 0) {
    ...
    Write-Host "`nNo new files were processed (all already cleaned)." -ForegroundColor Yellow
}

Write-Host "`nRun completed at $RunDateTime." -ForegroundColor Cyan
